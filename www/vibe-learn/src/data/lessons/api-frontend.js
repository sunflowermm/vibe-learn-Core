...
| 常考词 | 一句话 | 业务里长什么样 |
|--------|--------|----------------|
| **B/S · C/S** | 浏览器访问 vs 独立客户端 | Web 管理台 vs 桌面/App |
| **REST 资源化** | URL 表资源，方法表动作 | \`/orders/1\` + GET/PATCH；忌 \`/getOrder\` 满天飞 |
| **同步 vs 异步 API** | 立刻结果 vs 任务 ID 再查 | 导出报表、视频转码走异步 |
| **RPC vs HTTP** | 像调本地函数 vs 通用 Web 协议 | 内网 gRPC；对外 REST/OpenAPI |
| **WebSocket** | 全双工长连接 | IM、协作；前面常仍经网关/反代 |
| **OpenAPI / Swagger** | 接口契约文档 | 前后端并行、联调少扯皮 |
| **网关** | API 统一入口 | 鉴权、限流、路由、灰度（见反代课） |

业务一条链：前端 →（可选网关）→ 后端 API → DB；实时能力另开 WS；对内高频用 RPC。

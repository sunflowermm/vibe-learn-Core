...
| 常考词 | 一句话 | 业务里长什么样 |
|--------|--------|----------------|
| **递归 vs 迭代查询** | 本地 DNS 帮你问到底 vs 只告诉你下一步问谁 | 客户端通常做递归请求；解析器对外常迭代 |
| **权威 DNS / 本地 DNS** | 域名「官方答案」vs 你电脑用的解析器 | 改 A 记录在权威；用户「刷不到」多半是缓存 TTL |
| **TTL** | 缓存多久过期 | 切流、迁移 IP 要提前降 TTL |
| **NS / SOA** | 谁管这块域名 | 把域名迁到 Cloudflare = 改 NS |
| **DNS 污染 / 劫持** | 解析结果被篡改或异常 | 权威对、本地错；对比多解析器 |
| **证书链 / CA** | 叶证书 → 中间 → 根 | 缺中间证书 → 部分客户端报证书错误 |
| **SNI** | TLS 握手时带上要访问的域名 | 同 IP 多证书；反代按域名选后端 |
| **HSTS** | 强制以后只用 HTTPS | 防降级；配错难回退，需谨慎 |
| **对称 / 非对称加密** | 会话密钥快 / 证书与密钥交换 | TLS：握手用非对称（或等价），传数据用对称 |
